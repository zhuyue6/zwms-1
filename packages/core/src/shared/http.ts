import { get as zwmsGet, post as zwmsPost, useMiddleWare } from '@zwms/http';
...
useMiddleWare({
  httpRequestMiddleWare(config) {
    ...
    const projectCode = userInfo.value?.projectCode
    const XToken = userInfo.value?.dwToken
  ...
  httpResponseMiddleWare(response) {
    if (response.data?.code !== 0 && response.data?.errCode !== 0) {
      ...
      if ([403, 404, 405, 8001, 703].includes(response.data?.code ?? response.data?.errCode as number)) {
        router.push({
          path: '/login',
        })
      }
  ...
  httpResponseMiddleWareError(error) {
    ...
    if ([403, 404, 405].includes(error?.response?.data?.code ?? error?.response?.data?.errCode)) {
      ElMessage({
...
export async function get<T, P=any, D=any>(
  ...
  return zwmsGet(url, mergeAppCodeParams, data as any, loading)
}
...
export async function post<T, D=any, P=any>(
  ...
  return zwmsPost<T, D, P>(url, data as any, mergeAppCodeParams, loading)
}
