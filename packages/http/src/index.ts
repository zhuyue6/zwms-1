...
export function ResponseMiddleware<T, D = any, P = any>(
  mode: 'get' | 'post',
  url: string,
  data: D,
  params: P,
  ...
  return new Promise<AxiosResponse<ResponseData<T>, any>>((resolve, reject) => {
    ;(
      request<T, D, P>(mode, url, data, params) as Promise<
...
export async function get<T, P = any, D = any>(
  ...
  const responseData = await ResponseMiddleware<T, D, P>(
    'get',
    url,
    data as any,
    params as any,
